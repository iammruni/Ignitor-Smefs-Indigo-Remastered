...
namespace JKfHkSldw
{
  ...
  { 
      void lBhrQqzHst()
      { 
          bool lfjVRiJJfK = false;
          bool LudwuaLOHe = false;
          bool JYsAdkXuHL = false;
          bool LVWRFwgBFq = false;
          bool BFmBCNGWtH = false;
          bool xBNxmNljrf = false;
          bool KNBUypYLpy = false;
          bool tKJQcDkqyP = false;
          bool eBBzNKHYer = false;
          bool JwJYgmDSDM = false;
          bool otfuNmqxTi = false;
          bool btlbAIMCHm = false;
          bool hzgxoxAkip = false;
          bool zSnkakVFlZ = false;
          bool TJFuQPKhXX = false;
          bool aBKQdpMiGP = false;
          bool mGNdjIGABP = false;
          bool mHPgTXskgY = false;
          bool lRMLPxHXHj = false;
          bool XmzVmMwbLH = false;
          string HgZfKkTjoh = null;
          string WQiTPQXktP = null;
          string hqeGlEdAop = null;
          string IunEZxLZEu = null;
          string rlNqqYpWfq = null;
          string GxwEqQCTsF = null;
          string GTuzZbbesS = null;
          string FxfzXwcmON = null;
          string OHUyiKyiNJ = null;
          string tqfznYqOQP = null;
          string wLdMkJQxgu = null;
          string mOFfpzyxPP = null;
          string csRjXOQgLz = null;
          string EOVyQDdHMZ = null;
          string tBCSVflzIn = null;
          string ZtyBlCtsoW = null;
          string rjlgpYsGOm = null;
          string VqjoUbqyOr = null;
          string bRXWZKPTMO = null;
          string IjSohJPUbw = null;
          if(HgZfKkTjoh == wLdMkJQxgu){lfjVRiJJfK = true;}
          else if(wLdMkJQxgu == HgZfKkTjoh){otfuNmqxTi = true;}
          if(WQiTPQXktP == mOFfpzyxPP){LudwuaLOHe = true;}
          else if(mOFfpzyxPP == WQiTPQXktP){btlbAIMCHm = true;}
          if(hqeGlEdAop == csRjXOQgLz){JYsAdkXuHL = true;}
          else if(csRjXOQgLz == hqeGlEdAop){hzgxoxAkip = true;}
          if(IunEZxLZEu == EOVyQDdHMZ){LVWRFwgBFq = true;}
          else if(EOVyQDdHMZ == IunEZxLZEu){zSnkakVFlZ = true;}
          if(rlNqqYpWfq == tBCSVflzIn){BFmBCNGWtH = true;}
          else if(tBCSVflzIn == rlNqqYpWfq){TJFuQPKhXX = true;}
          if(GxwEqQCTsF == ZtyBlCtsoW){xBNxmNljrf = true;}
          else if(ZtyBlCtsoW == GxwEqQCTsF){aBKQdpMiGP = true;}
          if(GTuzZbbesS == rjlgpYsGOm){KNBUypYLpy = true;}
          else if(rjlgpYsGOm == GTuzZbbesS){mGNdjIGABP = true;}
          if(FxfzXwcmON == VqjoUbqyOr){tKJQcDkqyP = true;}
          if(OHUyiKyiNJ == bRXWZKPTMO){eBBzNKHYer = true;}
          if(tqfznYqOQP == IjSohJPUbw){JwJYgmDSDM = true;}
          while(VqjoUbqyOr == FxfzXwcmON){mHPgTXskgY = true;}
          while(bRXWZKPTMO == OHUyiKyiNJ){lRMLPxHXHj = true;}
          while(IjSohJPUbw == tqfznYqOQP){XmzVmMwbLH = true;}
          if(lfjVRiJJfK == true){lfjVRiJJfK = false;}
          if(LudwuaLOHe == true){LudwuaLOHe = false;}
          if(JYsAdkXuHL == true){JYsAdkXuHL = false;}
          if(LVWRFwgBFq == true){LVWRFwgBFq = false;}
          if(BFmBCNGWtH == true){BFmBCNGWtH = false;}
          if(xBNxmNljrf == true){xBNxmNljrf = false;}
          if(KNBUypYLpy == true){KNBUypYLpy = false;}
          if(tKJQcDkqyP == true){tKJQcDkqyP = false;}
          if(eBBzNKHYer == true){eBBzNKHYer = false;}
          if(JwJYgmDSDM == true){JwJYgmDSDM = false;}
          if(otfuNmqxTi == true){otfuNmqxTi = false;}
          if(btlbAIMCHm == true){btlbAIMCHm = false;}
          if(hzgxoxAkip == true){hzgxoxAkip = false;}
          if(zSnkakVFlZ == true){zSnkakVFlZ = false;}
          if(TJFuQPKhXX == true){TJFuQPKhXX = false;}
          if(aBKQdpMiGP == true){aBKQdpMiGP = false;}
          if(mGNdjIGABP == true){mGNdjIGABP = false;}
          if(mHPgTXskgY == true){mHPgTXskgY = false;}
          if(lRMLPxHXHj == true){lRMLPxHXHj = false;}
          if(XmzVmMwbLH == true){XmzVmMwbLH = false;}
      } 
  }
} 
...
namespace snGXVAIpS
{
  ...
  { 
      void ZQifVDSVpu()
      { 
          bool CbHcUoBZsp = false;
          bool OdUwgYbVby = false;
          bool bYECIWJuxR = false;
          bool nWxYWoXAJG = false;
          bool ZJHyqkakiP = false;
          bool pBYLcdWETy = false;
          bool fGcbBqfuYq = false;
          bool BaLVYRRFKW = false;
          bool wXMgamylws = false;
          bool SSgLZSgexA = false;
          bool EfMqOOoDmK = false;
          bool LQtwBHtuAN = false;
          bool styeYhtAuF = false;
          bool kmIKRPLUSM = false;
          bool yuykhBYjJS = false;
          bool YkdiGKggRf = false;
          bool TnHgZYIjdl = false;
          bool wYcnPywMxX = false;
          bool JJqcfzIZDH = false;
          bool hdClrJmQLI = false;
          string KuDSHBHDmV = null;
          string FEaqmPOpWE = null;
          string jLYolAAjeL = null;
          string CegkXEUziC = null;
          string zKVGLHTMHe = null;
          string PrnGmyWOgH = null;
          string AWYbTaXoTi = null;
          string tOGUFZpRcd = null;
          string fmbzuKqZxY = null;
          string gGSbBblsmk = null;
          string jdsXkAkTBN = null;
          string IEkCaNuYgI = null;
          string ixrtCmweSc = null;
          string nweqsBtDNt = null;
          string YXYjQbVQBn = null;
          string gMMdCmlyVy = null;
          string EGAXZLZtIY = null;
          string SqKljLXIdZ = null;
          string QedewajAoG = null;
          string PuhPoSWAKE = null;
          if(KuDSHBHDmV == jdsXkAkTBN){CbHcUoBZsp = true;}
          else if(jdsXkAkTBN == KuDSHBHDmV){EfMqOOoDmK = true;}
          if(FEaqmPOpWE == IEkCaNuYgI){OdUwgYbVby = true;}
          else if(IEkCaNuYgI == FEaqmPOpWE){LQtwBHtuAN = true;}
          if(jLYolAAjeL == ixrtCmweSc){bYECIWJuxR = true;}
          else if(ixrtCmweSc == jLYolAAjeL){styeYhtAuF = true;}
          if(CegkXEUziC == nweqsBtDNt){nWxYWoXAJG = true;}
          else if(nweqsBtDNt == CegkXEUziC){kmIKRPLUSM = true;}
          if(zKVGLHTMHe == YXYjQbVQBn){ZJHyqkakiP = true;}
          else if(YXYjQbVQBn == zKVGLHTMHe){yuykhBYjJS = true;}
          if(PrnGmyWOgH == gMMdCmlyVy){pBYLcdWETy = true;}
          else if(gMMdCmlyVy == PrnGmyWOgH){YkdiGKggRf = true;}
          if(AWYbTaXoTi == EGAXZLZtIY){fGcbBqfuYq = true;}
          else if(EGAXZLZtIY == AWYbTaXoTi){TnHgZYIjdl = true;}
          if(tOGUFZpRcd == SqKljLXIdZ){BaLVYRRFKW = true;}
          if(fmbzuKqZxY == QedewajAoG){wXMgamylws = true;}
          if(gGSbBblsmk == PuhPoSWAKE){SSgLZSgexA = true;}
          while(SqKljLXIdZ == tOGUFZpRcd){wYcnPywMxX = true;}
          while(QedewajAoG == fmbzuKqZxY){JJqcfzIZDH = true;}
          while(PuhPoSWAKE == gGSbBblsmk){hdClrJmQLI = true;}
          if(CbHcUoBZsp == true){CbHcUoBZsp = false;}
          if(OdUwgYbVby == true){OdUwgYbVby = false;}
          if(bYECIWJuxR == true){bYECIWJuxR = false;}
          if(nWxYWoXAJG == true){nWxYWoXAJG = false;}
          if(ZJHyqkakiP == true){ZJHyqkakiP = false;}
          if(pBYLcdWETy == true){pBYLcdWETy = false;}
          if(fGcbBqfuYq == true){fGcbBqfuYq = false;}
          if(BaLVYRRFKW == true){BaLVYRRFKW = false;}
          if(wXMgamylws == true){wXMgamylws = false;}
          if(SSgLZSgexA == true){SSgLZSgexA = false;}
          if(EfMqOOoDmK == true){EfMqOOoDmK = false;}
          if(LQtwBHtuAN == true){LQtwBHtuAN = false;}
          if(styeYhtAuF == true){styeYhtAuF = false;}
          if(kmIKRPLUSM == true){kmIKRPLUSM = false;}
          if(yuykhBYjJS == true){yuykhBYjJS = false;}
          if(YkdiGKggRf == true){YkdiGKggRf = false;}
          if(TnHgZYIjdl == true){TnHgZYIjdl = false;}
          if(wYcnPywMxX == true){wYcnPywMxX = false;}
          if(JJqcfzIZDH == true){JJqcfzIZDH = false;}
          if(hdClrJmQLI == true){hdClrJmQLI = false;}
      } 
  }
} 
...
namespace TGyzMHNcc
{
  ...
  { 
      void WxlmKQnjaw()
      { 
          bool olCEAjBDeX = false;
          bool BqMfESeush = false;
          bool TUQBTEPLiN = false;
          bool paJJxLmBnY = false;
          bool pnooBWeTky = false;
          bool tjsdsTVBju = false;
          bool UoURhZgChs = false;
          bool fcGTqeeazx = false;
          bool ZuajSFheQc = false;
          bool pkQsDBSXHa = false;
          bool obkrSQdYmO = false;
          bool ntBmVZNkyc = false;
          bool rClFDtMNQp = false;
          bool MHTYHxJiRC = false;
          bool bIrBoujdXd = false;
          bool mhYdhmofnX = false;
          bool DPIwCfaceX = false;
          bool QeTuBiJjQH = false;
          bool KipmDeiopU = false;
          bool HctsyLAwzI = false;
          string HXIAwHyzTm = null;
          string lHrBHsBtSO = null;
          string yTukynWMtK = null;
          string EQFZViacFI = null;
          string VDpwzRoMIj = null;
          string dRfFQmbXBI = null;
          string jlFJhUCReY = null;
          string RXoKstdzkg = null;
          string yKDKglkAIh = null;
          string GDbwXiygNd = null;
          string PWgjRFcVLV = null;
          string IblUKZitbu = null;
          string jBIwROwrlJ = null;
          string uxlHQbcSra = null;
          string JOnmYeQGlx = null;
          string ADERsNNjSC = null;
          string JjiCZLoeDh = null;
          string xPCAlUMVXl = null;
          string ypFnJrrUNC = null;
          string OrApRYelMN = null;
          if(HXIAwHyzTm == PWgjRFcVLV){olCEAjBDeX = true;}
          else if(PWgjRFcVLV == HXIAwHyzTm){obkrSQdYmO = true;}
          if(lHrBHsBtSO == IblUKZitbu){BqMfESeush = true;}
          else if(IblUKZitbu == lHrBHsBtSO){ntBmVZNkyc = true;}
          if(yTukynWMtK == jBIwROwrlJ){TUQBTEPLiN = true;}
          else if(jBIwROwrlJ == yTukynWMtK){rClFDtMNQp = true;}
          if(EQFZViacFI == uxlHQbcSra){paJJxLmBnY = true;}
          else if(uxlHQbcSra == EQFZViacFI){MHTYHxJiRC = true;}
          if(VDpwzRoMIj == JOnmYeQGlx){pnooBWeTky = true;}
          else if(JOnmYeQGlx == VDpwzRoMIj){bIrBoujdXd = true;}
          if(dRfFQmbXBI == ADERsNNjSC){tjsdsTVBju = true;}
          else if(ADERsNNjSC == dRfFQmbXBI){mhYdhmofnX = true;}
          if(jlFJhUCReY == JjiCZLoeDh){UoURhZgChs = true;}
          else if(JjiCZLoeDh == jlFJhUCReY){DPIwCfaceX = true;}
          if(RXoKstdzkg == xPCAlUMVXl){fcGTqeeazx = true;}
          if(yKDKglkAIh == ypFnJrrUNC){ZuajSFheQc = true;}
          if(GDbwXiygNd == OrApRYelMN){pkQsDBSXHa = true;}
          while(xPCAlUMVXl == RXoKstdzkg){QeTuBiJjQH = true;}
          while(ypFnJrrUNC == yKDKglkAIh){KipmDeiopU = true;}
          while(OrApRYelMN == GDbwXiygNd){HctsyLAwzI = true;}
          if(olCEAjBDeX == true){olCEAjBDeX = false;}
          if(BqMfESeush == true){BqMfESeush = false;}
          if(TUQBTEPLiN == true){TUQBTEPLiN = false;}
          if(paJJxLmBnY == true){paJJxLmBnY = false;}
          if(pnooBWeTky == true){pnooBWeTky = false;}
          if(tjsdsTVBju == true){tjsdsTVBju = false;}
          if(UoURhZgChs == true){UoURhZgChs = false;}
          if(fcGTqeeazx == true){fcGTqeeazx = false;}
          if(ZuajSFheQc == true){ZuajSFheQc = false;}
          if(pkQsDBSXHa == true){pkQsDBSXHa = false;}
          if(obkrSQdYmO == true){obkrSQdYmO = false;}
          if(ntBmVZNkyc == true){ntBmVZNkyc = false;}
          if(rClFDtMNQp == true){rClFDtMNQp = false;}
          if(MHTYHxJiRC == true){MHTYHxJiRC = false;}
          if(bIrBoujdXd == true){bIrBoujdXd = false;}
          if(mhYdhmofnX == true){mhYdhmofnX = false;}
          if(DPIwCfaceX == true){DPIwCfaceX = false;}
          if(QeTuBiJjQH == true){QeTuBiJjQH = false;}
          if(KipmDeiopU == true){KipmDeiopU = false;}
          if(HctsyLAwzI == true){HctsyLAwzI = false;}
      } 
  }
} 
...
namespace LNCWXUxiH
{
  ...
  { 
      void YUHguEwkoz()
      { 
          bool TgaSMYfkYN = false;
          bool YESOXqgcPJ = false;
          bool sAMbmOnwKc = false;
          bool zJpHuCjtGt = false;
          bool VczIEgXNpX = false;
          bool lUNADTUzey = false;
          bool jiauNPNLTU = false;
          bool GmVVmQAEWF = false;
          bool paGmpeupDa = false;
          bool hkOHJmCDto = false;
          bool TMsUQzyrNm = false;
          bool SPbYHJbmFi = false;
          bool TXHrmVXOMA = false;
          bool OxJhldglVp = false;
          bool fCcQIRIoAz = false;
          bool KOxKUuEBgt = false;
          bool bjfMYUGtlc = false;
          bool boQODQupiV = false;
          bool NiNMozAyMq = false;
          bool YfEbOuWPqm = false;
          string jWFHVrKOSC = null;
          string yVgoTJiIbx = null;
          string lceFOlauKS = null;
          string dfSnsCfSrE = null;
          string bZBEUOTdDj = null;
          string IPFNPHrAsi = null;
          string ROEAYRDsuR = null;
          string dPPXVMaZjz = null;
          string RxrZJNaEXl = null;
          string JeYJlMUBfC = null;
          string KahkFsAgrD = null;
          string gRPsQUWepK = null;
          string qzVJHyqFkI = null;
          string rYsgnqOGTQ = null;
          string NsMUErPfrR = null;
          string ppKVLLFXdG = null;
          string CtuiKHXhEZ = null;
          string FQKZwxmAEP = null;
          string iKMSSBOXVc = null;
          string lqbNOTxdJx = null;
          if(jWFHVrKOSC == KahkFsAgrD){TgaSMYfkYN = true;}
          else if(KahkFsAgrD == jWFHVrKOSC){TMsUQzyrNm = true;}
          if(yVgoTJiIbx == gRPsQUWepK){YESOXqgcPJ = true;}
          else if(gRPsQUWepK == yVgoTJiIbx){SPbYHJbmFi = true;}
          if(lceFOlauKS == qzVJHyqFkI){sAMbmOnwKc = true;}
          else if(qzVJHyqFkI == lceFOlauKS){TXHrmVXOMA = true;}
          if(dfSnsCfSrE == rYsgnqOGTQ){zJpHuCjtGt = true;}
          else if(rYsgnqOGTQ == dfSnsCfSrE){OxJhldglVp = true;}
          if(bZBEUOTdDj == NsMUErPfrR){VczIEgXNpX = true;}
          else if(NsMUErPfrR == bZBEUOTdDj){fCcQIRIoAz = true;}
          if(IPFNPHrAsi == ppKVLLFXdG){lUNADTUzey = true;}
          else if(ppKVLLFXdG == IPFNPHrAsi){KOxKUuEBgt = true;}
          if(ROEAYRDsuR == CtuiKHXhEZ){jiauNPNLTU = true;}
          else if(CtuiKHXhEZ == ROEAYRDsuR){bjfMYUGtlc = true;}
          if(dPPXVMaZjz == FQKZwxmAEP){GmVVmQAEWF = true;}
          if(RxrZJNaEXl == iKMSSBOXVc){paGmpeupDa = true;}
          if(JeYJlMUBfC == lqbNOTxdJx){hkOHJmCDto = true;}
          while(FQKZwxmAEP == dPPXVMaZjz){boQODQupiV = true;}
          while(iKMSSBOXVc == RxrZJNaEXl){NiNMozAyMq = true;}
          while(lqbNOTxdJx == JeYJlMUBfC){YfEbOuWPqm = true;}
          if(TgaSMYfkYN == true){TgaSMYfkYN = false;}
          if(YESOXqgcPJ == true){YESOXqgcPJ = false;}
          if(sAMbmOnwKc == true){sAMbmOnwKc = false;}
          if(zJpHuCjtGt == true){zJpHuCjtGt = false;}
          if(VczIEgXNpX == true){VczIEgXNpX = false;}
          if(lUNADTUzey == true){lUNADTUzey = false;}
          if(jiauNPNLTU == true){jiauNPNLTU = false;}
          if(GmVVmQAEWF == true){GmVVmQAEWF = false;}
          if(paGmpeupDa == true){paGmpeupDa = false;}
          if(hkOHJmCDto == true){hkOHJmCDto = false;}
          if(TMsUQzyrNm == true){TMsUQzyrNm = false;}
          if(SPbYHJbmFi == true){SPbYHJbmFi = false;}
          if(TXHrmVXOMA == true){TXHrmVXOMA = false;}
          if(OxJhldglVp == true){OxJhldglVp = false;}
          if(fCcQIRIoAz == true){fCcQIRIoAz = false;}
          if(KOxKUuEBgt == true){KOxKUuEBgt = false;}
          if(bjfMYUGtlc == true){bjfMYUGtlc = false;}
          if(boQODQupiV == true){boQODQupiV = false;}
          if(NiNMozAyMq == true){NiNMozAyMq = false;}
          if(YfEbOuWPqm == true){YfEbOuWPqm = false;}
      } 
  }
} 
...
namespace cJHjeKECl
{
  ...
  { 
      void ytYXySYxdi()
      { 
          bool PVDPfTuusD = false;
          bool XdpkIXInEw = false;
          bool UDcLqwWatY = false;
          bool YjxpKcPFqJ = false;
          bool GwfUBilgyu = false;
          bool kQXUjLsqXi = false;
          bool wTATdngAZe = false;
          bool IDwupYXHKI = false;
          bool KobzmHMbFz = false;
          bool jXgIMluLqN = false;
          bool BlHSlhUbeL = false;
          bool mkfPYHrBgV = false;
          bool aFtaujVmhE = false;
          bool cLQxGYhIME = false;
          bool mjxtLyrQjE = false;
          bool tseUlwqgXC = false;
          bool MgsZhbUiIA = false;
          bool ZinbhqHAOw = false;
          bool rEDcKjguLK = false;
          bool gBkKrGpWLj = false;
          string dftgQwmPXZ = null;
          string JGxJiOjuFK = null;
          string YmdHVZzMXt = null;
          string nKGZTIPtXm = null;
          string ATIoaFunEc = null;
          string msFdhpCmKq = null;
          string xZrclzzWpZ = null;
          string zpXRKFengP = null;
          string eLItGtEcrc = null;
          string GcigpZaeDb = null;
          string GXxedxVmSb = null;
          string LNGtLlRyWq = null;
          string tVeMTVGqbk = null;
          string dzkFJGNcPJ = null;
          string YQJPRrWnUE = null;
          string HcILgjskLI = null;
          string peLhxLcAkE = null;
          string YnhMSdMOua = null;
          string zSVXcBqYXY = null;
          string NbRHStEbRF = null;
          if(dftgQwmPXZ == GXxedxVmSb){PVDPfTuusD = true;}
          else if(GXxedxVmSb == dftgQwmPXZ){BlHSlhUbeL = true;}
          if(JGxJiOjuFK == LNGtLlRyWq){XdpkIXInEw = true;}
          else if(LNGtLlRyWq == JGxJiOjuFK){mkfPYHrBgV = true;}
          if(YmdHVZzMXt == tVeMTVGqbk){UDcLqwWatY = true;}
          else if(tVeMTVGqbk == YmdHVZzMXt){aFtaujVmhE = true;}
          if(nKGZTIPtXm == dzkFJGNcPJ){YjxpKcPFqJ = true;}
          else if(dzkFJGNcPJ == nKGZTIPtXm){cLQxGYhIME = true;}
          if(ATIoaFunEc == YQJPRrWnUE){GwfUBilgyu = true;}
          else if(YQJPRrWnUE == ATIoaFunEc){mjxtLyrQjE = true;}
          if(msFdhpCmKq == HcILgjskLI){kQXUjLsqXi = true;}
          else if(HcILgjskLI == msFdhpCmKq){tseUlwqgXC = true;}
          if(xZrclzzWpZ == peLhxLcAkE){wTATdngAZe = true;}
          else if(peLhxLcAkE == xZrclzzWpZ){MgsZhbUiIA = true;}
          if(zpXRKFengP == YnhMSdMOua){IDwupYXHKI = true;}
          if(eLItGtEcrc == zSVXcBqYXY){KobzmHMbFz = true;}
          if(GcigpZaeDb == NbRHStEbRF){jXgIMluLqN = true;}
          while(YnhMSdMOua == zpXRKFengP){ZinbhqHAOw = true;}
          while(zSVXcBqYXY == eLItGtEcrc){rEDcKjguLK = true;}
          while(NbRHStEbRF == GcigpZaeDb){gBkKrGpWLj = true;}
          if(PVDPfTuusD == true){PVDPfTuusD = false;}
          if(XdpkIXInEw == true){XdpkIXInEw = false;}
          if(UDcLqwWatY == true){UDcLqwWatY = false;}
          if(YjxpKcPFqJ == true){YjxpKcPFqJ = false;}
          if(GwfUBilgyu == true){GwfUBilgyu = false;}
          if(kQXUjLsqXi == true){kQXUjLsqXi = false;}
          if(wTATdngAZe == true){wTATdngAZe = false;}
          if(IDwupYXHKI == true){IDwupYXHKI = false;}
          if(KobzmHMbFz == true){KobzmHMbFz = false;}
          if(jXgIMluLqN == true){jXgIMluLqN = false;}
          if(BlHSlhUbeL == true){BlHSlhUbeL = false;}
          if(mkfPYHrBgV == true){mkfPYHrBgV = false;}
          if(aFtaujVmhE == true){aFtaujVmhE = false;}
          if(cLQxGYhIME == true){cLQxGYhIME = false;}
          if(mjxtLyrQjE == true){mjxtLyrQjE = false;}
          if(tseUlwqgXC == true){tseUlwqgXC = false;}
          if(MgsZhbUiIA == true){MgsZhbUiIA = false;}
          if(ZinbhqHAOw == true){ZinbhqHAOw = false;}
          if(rEDcKjguLK == true){rEDcKjguLK = false;}
          if(gBkKrGpWLj == true){gBkKrGpWLj = false;}
      } 
  }
} 
